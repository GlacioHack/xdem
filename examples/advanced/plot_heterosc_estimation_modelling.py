...
and rarely accounted for in DEM studies (see :ref:`accuracy-precision`).
Quantifying elevation heteroscedasticity is essential to
...
:func:`~xdem.spatialstats.infer_heteroscedasticity_from_stable`
exemplified in :ref:`sphx_glr_basic_examples_plot_infer_heterosc.py`.
...
# %%
# We use :func:`xdem.spatialstats.nd_binning` to perform N-dimensional binning on all
# those terrain variables, with uniform
# bin length divided by 30. We use the NMAD as a robust measure of
# `statistical dispersion <https://en.wikipedia.org/wiki/Statistical_dispersion>`_
...
xdem.spatialstats.plot_1d_binning(
    df_bin, var_name="slope", statistic_name="nmad", label_var="Slope (degrees)", label_statistic="NMAD of dh (m)",
)
...
xdem.spatialstats.plot_1d_binning(df_bin, "aspect", "nmad", "Aspect (degrees)", "NMAD of dh (m)")

# %%
# There is no variability with the aspect that shows a dispersion averaging 2-3 meters,
# i.e. that of the complete sample.
#
# What about the plan curvature?

xdem.spatialstats.plot_1d_binning(df_bin, "planc", "nmad", "Planform curvature (100 m$^{-1}$)", "NMAD of dh (m)")
...
xdem.spatialstats.plot_1d_binning(df_bin, "profc", "nmad", "Profile curvature (100 m$^{-1}$)", "NMAD of dh (m)")
...
# %%
# The plan curvature shows a similar relation. Those are symmetrical with 0,
# and almost equal for both types of curvature.
...
xdem.spatialstats.plot_1d_binning(df_bin, "maxc", "nmad", "Maximum absolute curvature (100 m$^{-1}$)", "NMAD of dh (m)")
...
print(
    f"The spread of elevation difference is {xdem.spatialstats.nmad(dh_arr):.2f} "
    f"compared to a mean predicted elevation error of {np.nanmean(dh_err_stable):.2f}.",
)
...
for s, c in [(0.0, 0.1), (50.0, 0.1), (0.0, 20.0), (50.0, 20.0)]:
    print(
        f"Elevation measurement error for slope of {s:.0f} degrees, "
        f"curvature of {c / 100:.2f} m-1: {dh_err_fun((s, c)):.1f}"
        f" meters.",
    )
