...
def test_init_topo_summary(get_topo_inputs_config, tmp_path, list_default_terrain_attributes):
    ...
    assert Path(tmp_path / "png").joinpath("dem.png").exists()
...
def test_run(get_topo_inputs_config, tmp_path):
    ...
    assert Path(tmp_path / "csv").joinpath("stats_dem.csv").exists()
    assert Path(tmp_path / "csv").joinpath("stats_dem_mask.csv").exists()
