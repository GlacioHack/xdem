...
tba = Raster(examples.get_path("longyearbyen_tba_dem"))
glacier_mask = Vector(examples.get_path("longyearbyen_glacier_outlines"))

inlier_mask = ~glacier_mask.create_mask(ref)
...
def test_biascorr__errors(load_examples) -> None:
...
def test_biascorr__fit_1d(fit_args, fit_func, fit_optimizer, capsys, load_examples) -> None:
    ...
    bcorr.apply(elev=tba, bias_vars=bias_vars_dict)
...
def test_biascorr__fit_2d(fit_args, fit_func, fit_optimizer, load_examples) -> None:
    """Test the _fit_func and apply_func methods of BiasCorr for the fit case (called by all its subclasses)."""
...
def test_biascorr__bin_1d(fit_args, bin_sizes, bin_statistic, load_examples) -> None:
    """Test the _fit_func and apply_func methods of BiasCorr for the fit case (called by all its subclasses)."""
...
@pytest.mark.parametrize("bin_statistic", [np.median, np.nanmean])  # type: ignore
def test_biascorr__bin_2d(fit_args, bin_sizes, bin_statistic, load_examples) -> None:
    """Test the _fit_func and apply_func methods of BiasCorr for the fit case (called by all its subclasses)."""
...
@pytest.mark.parametrize("bin_statistic", [np.median, np.nanmean])  # type: ignore
def test_biascorr__bin_and_fit_1d(fit_args, fit_func, fit_optimizer, bin_sizes, bin_statistic, load_examples) -> None:
...
@pytest.mark.parametrize("bin_sizes", (10, {"elevation": (0, 500, 1000), "slope": (0, 20, 40)}))  # type: ignore
@pytest.mark.parametrize("bin_statistic", [np.median, np.nanmean])  # type: ignore
def test_biascorr__bin_and_fit_2d(fit_args, fit_func, fit_optimizer, bin_sizes, bin_statistic, load_examples) -> None:
    """Test the _fit_func and apply_func methods of BiasCorr for the bin_and_fit case (called by all subclasses)."""
...
def test_directionalbias__synthetic(fit_args, angle, nb_freq, load_examples) -> None:
    """Test the subclass DirectionalBias with synthetic data."""

    ref, tba, _, _, _ = load_examples
    # Get along track
    xx = gu.raster.get_xy_rotated(ref, along_track_angle=angle)[0]
    ...
    bias_dem = ref - synthetic_bias.reshape(np.shape(ref.data))

    # For debugging
    if PLOT:
        synth = ref.copy(new_array=synthetic_bias.reshape(np.shape(ref.data)))
        ...
        dirbias.fit(reference_elev=ref, to_be_aligned_elev=bias_dem, subsample=10000, random_state=42)
    ...
    assert np.nanvar((corrected_dem - ref) / np.nanstd(synthetic_bias)) < 0.01


def test_deramp() -> None:
...
def test_deramp__synthetic(fit_args, order: int, load_examples) -> None:
    """Run the deramp for varying polynomial orders using a synthetic elevation difference."""

    ref, tba, _, _, _ = load_examples

    # Get coordinates
    xx, yy = np.meshgrid(np.arange(0, ref.shape[1]), np.arange(0, ref.shape[0]))
...
@pytest.mark.parametrize("fit_args", all_fit_args)  # type: ignore
def test_terrainbias__synthetic(fit_args, load_examples) -> None:
    """Test the subclass TerrainBias."""

    ref, tba, _, _, _ = load_examples

    # Get maximum curvature
    maxc = xdem.terrain.get_terrain_attribute(ref, attribute="maximum_curvature")

    # Create a bias depending on bins
    synthetic_bias = np.zeros(np.shape(ref.data))
