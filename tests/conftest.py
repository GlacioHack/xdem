...
@pytest.fixture(scope="session")
def get_terrainattr_richdem(raster_to_rda):
...
@pytest.fixture(scope="session")
def get_terrain_attribute_richdem(get_terrainattr_richdem):
    def _get_terrain_attribute_richdem(
        ...
        if make_slope:
            terrain_attributes["slope"] = get_terrainattr_richdem(dem, attribute="slope_radians")

        if make_aspect:
            # The aspect of RichDEM is returned in degrees, we convert to radians to match the others
            terrain_attributes["aspect"] = np.deg2rad(get_terrainattr_richdem(dem, attribute="aspect"))
            # For flat slopes, RichDEM returns a 90° aspect by default, while GDAL return a 180° aspect
            # We stay consistent with GDAL
            slope_tmp = get_terrainattr_richdem(dem, attribute="slope_radians")
        ...
        if make_curvature:
            terrain_attributes["curvature"] = get_terrainattr_richdem(dem, attribute="curvature")

        if make_planform_curvature:
            terrain_attributes["planform_curvature"] = get_terrainattr_richdem(dem, attribute="planform_curvature")

        if make_profile_curvature:
            terrain_attributes["profile_curvature"] = get_terrainattr_richdem(dem, attribute="profile_curvature")
