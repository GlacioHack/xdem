...
df_ns = xdem.spstats.nd_binning(ddem.data.ravel(),list_var=[slope.ravel()],list_var_names=['slope'])

# Sample empirical variogram
df_vgm = xdem.spstats.sample_multirange_empirical_variogram(dh=ddem.data, nsamp=1000, nrun=20, nproc=10, maxlag=10000)

# Fit single-range spherical model
fun, coefs = xdem.spstats.fit_model_sum_vgm(['Sph'], emp_vgm_df=df_vgm)

# Fit sum of triple-range spherical model
fun2, coefs2 = xdem.spstats.fit_model_sum_vgm(['Sph', 'Sph', 'Sph'], emp_vgm_df=df_vgm)
