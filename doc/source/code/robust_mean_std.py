...
# Add outliers
dh_outliers = np.concatenate((np.repeat(-34, 600), np.repeat(-33, 1800), np.repeat(-32, 3600), np.repeat(-31, 8500),
                             np.repeat(-30, 15000), np.repeat(-29, 9000), np.repeat(-28, 3800),
                             np.repeat(-27, 1900), np.repeat(-26, 700)))
...
ax.vlines(x=[mean_dh_in - std_dh_in, mean_dh_in + std_dh_in, median_dh_in - nmad_dh_in, median_dh_in + nmad_dh_in], ymin=0, ymax=max_count, colors=["gray", "gray", "black", "black"], linestyles="dashed")

ax.vlines(x=[mean_dh, median_dh], ymin=0, ymax=max_count, colors=["red", "darkred"])
ax.vlines(x=[mean_dh - std_dh, mean_dh + std_dh, median_dh - nmad_dh, median_dh + nmad_dh], ymin=0, ymax=max_count, colors=["red", "red", "darkred", "darkred"], linestyles="dashed")
...
handles = [Rectangle((0, 0), 1, 1, color=h1[-1][0].get_facecolor(), alpha=1),
